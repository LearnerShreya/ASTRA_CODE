...
arduino2 = serial.Serial('COM8',9600)
# Load environment variableso
load_dotenv()
...
def chat_with_gemini(prompt):
    try:
        ...
        elif "weather" in prompt or "temperature in" in prompt or "forecast for" in prompt:
            city = None
            match = re.search(r'(?:weather|temperature in|forecast for)(?:\s+(?:in|at|of))?\s+([a-zA-Z\s]+)', prompt)
            if match:
                city = match.group(1).strip()
            else:
                city = prompt.strip().split()[-1]
    ...
    while True:
        ...
        if user_input:
            ...
            if stop_words & words:
                ...
                print("ASTRA: Hello, nice to meet you!")
                engine.say("Hello, nice to meet you!")
                engine.runAndWait()

            elif "your name" in user_input or "who are you" in user_input:
                print("ASTRA: My name is ASTRA! I am your AI assistant.")
                engine.say("My name is ASTRA! I am your AI assistant.")
                engine.runAndWait()
